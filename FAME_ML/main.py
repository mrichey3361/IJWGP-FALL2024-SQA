...
logger = main_logger.giveMeLoggingObject()

def giveTimeStamp():
  tsObj = time.time()
  strToret = datetime.datetime.fromtimestamp(tsObj).strftime(constants.TIME_FORMAT)
  logger.info(f"Timestamp generated: {strToret}")
  return strToret
...
def getCSVData(dic_, dir_repo):
	...
	for TEST_ML_SCRIPT in dic_:
		...
		logger.info(f"Data load count for Section 1.1a: {data_load_counta}")

		# Section 1.1b
		data_load_countb = lint_engine.getDataLoadCountb( TEST_ML_SCRIPT )
		logger.info(f"Data load count for Section 1.1b: {data_load_countb}")
		# Section 1.1c
		data_load_countc = lint_engine.getDataLoadCountc( TEST_ML_SCRIPT )
		logger.info(f"Data load count for Section 1.1c: {data_load_countc}")
		# Section 1.2a
		model_load_counta = lint_engine.getModelLoadCounta( TEST_ML_SCRIPT )
		logger.info(f"Model load count for Section 1.2a: {model_load_counta}")

		# Section 1.2b
		model_load_countb = lint_engine.getModelLoadCountb( TEST_ML_SCRIPT )
		logger.info(f"Model load count for Section 1.2b: {model_load_countb}")
		# Section 1.2c
		model_load_countc = lint_engine.getModelLoadCountc( TEST_ML_SCRIPT )
		logger.info(f"Model load count for Section 1.2c: {model_load_countc}")
		# Section 1.2d
		model_load_countd = lint_engine.getModelLoadCountd( TEST_ML_SCRIPT )
		logger.info(f"Model load count for Section 1.2d: {model_load_countd}")
		# Section 2.1a
		data_download_counta = lint_engine.getDataDownLoadCount( TEST_ML_SCRIPT )
		logger.info(f"Data download count for Section 2.1a: {data_download_counta}")
		# Section 2.1b
		data_download_countb = lint_engine.getDataDownLoadCountb( TEST_ML_SCRIPT )
		logger.info(f"Data download count for Section 2.1b: {data_download_countb}")
		...
		logger.info(f"Model label count for Section 3.2a: {model_label_counta}")
		# Section 3.2b
		# # skipping as per https://github.com/paser-group/MLForensics/blob/farzana/Verb.Object.Mapping.md
		...
		logger.info(f"Model output count for Section 3.3a: {model_output_counta}")

		# Section 3.3b
		model_output_countb = lint_engine.getModelOutputCountb( TEST_ML_SCRIPT )
		logger.info(f"Model output count for Section 3.3b: {model_output_countb}")
		...
		logger.info(f"Data pipeline count for Section 4.1: {data_pipeline_counta}")
		# Section 4.2
		data_pipeline_countb = lint_engine.getDataPipelineCountb( TEST_ML_SCRIPT )
		logger.info(f"Data pipeline count for Section 4.2: {data_pipeline_countb}")
		# Section 4.3
		data_pipeline_countc = lint_engine.getDataPipelineCountc( TEST_ML_SCRIPT )
		logger.info(f"Data pipeline count for Section 4.3: {data_pipeline_countc}")
		# Section 4.4
		# # skipping as per https://github.com/paser-group/MLForensics/blob/farzana/Verb.Object.Mapping.md
		...
		logger.info(f"Environment count for Section 5.1a: {environment_counta}")
		# Section 5.1b
		# # skipping as per https://github.com/paser-group/MLForensics/blob/farzana/Verb.Object.Mapping.md
		...
		logger.info(f"State observe count for Section 5.2: {state_observe_count}")
...
def getAllPythonFilesinRepo(path2dir):
	...
	for root_, dirnames, filenames in os.walk(path2dir):
		for file_ in filenames:
			...
			if( os.path.exists( full_path_file ) ):
				logger.info(f"Full path file: {full_path_file}")
	...
	logger.info(f"Python file found: {len(valid_list)}")
	return valid_list
...
def runFameML(inp_dir, csv_fil):
	...
	logger.info(f"List of subfolders with paths: {list_subfolders_with_paths}")
...
if __name__=='__main__':
	...
	if command_line_flag:
		...
		logger.eval(f"Directory path: {dir_path}")
		...
		full_dict  = runFameML(repo_dir, output_csv)
		logger.infor(f"Full dictionary: {full_dict}")
